...
gsap.set('.right-feather', {scale: 0, transformOrigin: 'center'});
home.addEventListener('click', () => {
    gsap.fromTo('.home-svg', {scale: 1}, {scale: 0.9, yoyo: true, repeat: 1});
    gsap.fromTo('.feather', {y: -5, scale: 0}, {y: 20, scale: 1.5, duration: 1, stragger: 0.2});
    gsap.fromTo('.right-feather', {x: 0}, {x: 6});
})

// Notification the transform origin
...
notification.addEventListener('click', () => {
    gsap.fromTo('.bell', {rotation: -5}, {rotation: 0, duration: 2, ease: 'elastic.out(3, 0.2)'});
    gsap.fromTo('.ringer', {rotation: -3, x: 0.5}, {x: 0, rotation: 0, duration: 1, ease: 'elastic.out(3, 0.2)'});
    gsap.fromTo('.wave', {scale: 0, opacity: 1}, {scale: 1.3, opacity: 0, duration: 1});
});

//Message the transform origin
